...
```{r cache=TRUE}
RISE2 <- get_tidy_audio_analysis("1FpVJ7HpZInE2GvhVE2TwT")
LOL23 <- RISE2 %>%
  tempogram(window_size = 8, hop_size = 1, cyclic = FALSE) %>%
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
...
RISE1 <- get_tidy_audio_analysis("1FpVJ7HpZInE2GvhVE2TwT")
LOL13 <- RISE1 %>%
  tempogram(window_size = 8, hop_size = 1, cyclic = TRUE) %>%
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
...
### Can we classify gaming music with **unsupervised** classifier?
...
dasddadd <- XP013 %>% 
  arrange(index) %>%
  ggplot(aes(x = playlist,fill= playlist)) +
  geom_boxplot() +
  coord_flip() 
